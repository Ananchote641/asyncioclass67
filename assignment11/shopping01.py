...
async def customer_generation(queue: Queue, customers: int):
    ...
    while True:
        ...
        customer_count = customer_count + len(customers_list)
...
async def main():
    customer_queue = asyncio.Queue(2)
    ...
    # รอให้ทุกแคชเชียร์ทำงานจนลูกค้าทั้งหมดเช็คเอาท์เสร็จ
    await asyncio.gather(customer_producer, *cashiers)
    await customer_queue.join()  # รอจนกว่าลูกค้าทั้งหมดจะถูกเช็คเอาท์
